...
namespace BZGUI
{
    class Task1_测试 : XTask
    {
        #region Definition
        private static int tempi = 0;
        private List<string[]> ls = new List<string[]>();
        private string[] CCD_data;
        ...
        {
            //if (Globals.settingMachineInfo.什么机器 == WhichMachine.IDPM)
            {
                CCD1_Close();
                Scanner_Close();
            }
            ...
            while (true) 
            {
                ...
                if (!PVar.MacHold)    //暂停状态时不扫描
                {
                    switch ((StationRunMode)runMode)
                    {
                        ...
                        case StationRunMode .CCD触发:
                            string ccd_data="";
                            string cmd_ccd = "T1,SN";
                            if (Globals.TCPIP_CCD1.IsOn)
                            {
                                Globals.TCPIP_CCD1.SendAndGetReply(cmd_ccd, out ccd_data, 1000);
                            }
                            SetStep(ccd_data, Mycolor.None);
                            { runMode = StationRunMode.自动运行; }
                            break;
                        case StationRunMode.条码枪触发:
                            string Scanner_data="";
                            string cmd_Scanner = "+";
                            if (Globals.TCPIP_Scanner.IsOn)
                            {
                                Globals.TCPIP_Scanner.SendAndGetReply(cmd_Scanner, out Scanner_data, 1000);
                            }
                            SetStep(Scanner_data, Mycolor.None);
                            { runMode = StationRunMode.自动运行; }
                            break;
        ...
        private void DryRun()
        {
            ...
            switch (AutoRunstep)
            {
                ...
                case 70:
                    ...
                    { AutoRunstep = (Globals.settingMachineInfo.什么机器 == WhichMachine.IPDM) ? 71 : 80; }
                    break;
                case 71:
                    Gg.AbsMotion(0, BVar.MMSY, mFunction.Pos.TeachAxis1[0, 1], PVar.ParAxis.Speed[BVar.MMSY]);
                    TimeOut = API.GetTickCount();
                    AutoRunstep = 72;
                    break;
                case 72:
                    if (Gg.ZSPD(0, BVar.MMSY))
                    {
                        SetStep("测试y轴运动到条码读取位置成功…", Mycolor.None);
                        TimeOut = API.GetTickCount();
                        AutoRunstep = 73;
                    }
                    break;
                ...
                case 74:
                    string sendtemp = "+";
                    if (Globals.TCPIP_Scanner.IsOn) Globals.TCPIP_Scanner.SendAndGetReply(sendtemp, out Barcode_data, 2000);
                    TimeOut = API.GetTickCount();
                    AutoRunstep = 75;
                    break;
                case 75:
                    ...
                    break;
                case 79:
                    if (Barcode_data == "") SetStep("条码扫描失败!", Mycolor.Red);                   
                        AutoRunstep = 80;
                    break;
                case 80://测试位置
                ...
                case 100:
                    if (API.GetTickCount() - TimeOut > 300)
                    { AutoRunstep =(Globals.settingMachineInfo.什么机器 == WhichMachine.IPDM)? 195:110; }
                    break;
                case 110:
                ...
                case 190:
                    ...
                    break;
                case 195:
                    string sendtemp1 = "T1,SN";
                    if (Globals.TCPIP_CCD1.IsOn)
                        Globals.TCPIP_CCD1.SendAndGetReply(sendtemp1, out CCD_data, 2000);
                    AutoRunstep = 196;
                    break;
                case 196:
                    if (Globals.TCPIP_CCD1.IsOn)
                        if (CCD_data[0] == "") SetStep("CCD触发失败!", Mycolor.Red);
                    AutoRunstep = 200;
                    break;
                case 200:
                    uint pck;
                ...
                case 210:
                    ...
                    if (Globals.settingMachineInfo.什么机器 == WhichMachine.IPDM)
                    {
                        ...
                        double Right1 = DataManager.Instance.CurrentCheckData.IPD_Left2 = rd.Next(-1, 1) / 100.0 + rd.Next(-1, 1) / 1000.00;
                        double Right2 = DataManager.Instance.CurrentCheckData.IPD_Right2 = rd.Next(-1, 1) / 100.0 + rd.Next(-1, 1) / 1000.00;
            ...
            if (AutoRunstepLast != AutoRunstep) { Globals.WriteLog(AutoRunstepLast.ToString(), PVar.BZ_TaskLogPath + "\\1.txt"); }//存log
        ...
        private void AutoRun()
        {
            ...
            switch (AutoRunstep)
            {
                case 10:
                    ...
                    AutoRunstep = (Globals.settingMachineInfo.什么机器 == WhichMachine.IPDM) ? 11 : 20;
                    break;
                #region IDPM扫条码开始
                case 11:
                    Gg.AbsMotion(0, BVar.MMSY, mFunction.Pos.TeachAxis1[0, 1], PVar.ParAxis.Speed[BVar.MMSY]);
                    TimeOut = API.GetTickCount();
                    SetStep("测试Y轴运动到扫条码位置开始", Mycolor.None);
                    AutoRunstep = 12;
                    break;
                case 12:
                    if (Gg.ZSPD(0, BVar.MMSY))
                    {
                        SetStep("测试Y轴运动到达扫条码位置", Mycolor.None);
                        TimeOut = API.GetTickCount();
                    }
                    AutoRunstep = 13;
                    break;
                case 13:
                    string sendtemp = "+";
                    Globals.TCPIP_PDCA.SendAndGetReply(sendtemp,out Barcode_data,2000);
                    AutoRunstep = 14;
                    break;
                case 14:
                    if (Barcode_data == "")
                    {
                        SetStep("条码扫描失败，测试Y轴返回待机位置开始", Mycolor.Red );
                        AutoRunstep = 320;
                    }
                    else 
                    { 
        ...
        private void MMS_Test()
        {
            switch (AutoRunstep)
            {
                ...
                case 240:
                    ...
                    path = PVar.BZ_DataPath + "MMS Data-" + Globals.settingMachineInfo.机器编号 + "\\Report Data\\"
                    ...
                    string pathtemp = path + DateTime.Now.ToString("yyyyMMdd") + "_Report_Data.csv";
                    if (File.Exists(pathtemp) == false)
                    {
                        CsvServer.Instance.WriteLine(pathtemp, "StartDate,StartTime,SN,Result,PDCA_Result,X,Y,A,CC,Operator_ID,Mode,TestSeriesID,Prioriyt");
                    }
        ...
        private FunctionStatus PDCA_Bali(string[] Test_data)//1D数组，和AE limit的count和项一样,调用的时候把OPID 版本等都包括在数组中
        {
            ...
            try
            {
                ...
                {
                    WritePDCALog(PVar.BZ_DataPath + "MMS Data-" + Globals.settingMachineInfo.机器编号 + "\\PDCA Data\\" + DateTime.Now.ToString("yyyyMMdd"), tempLogStr, DataManager.Instance.CurrentCheckData.SN);
        ...
        #region CCD通讯
        private bool CCD1_Connect()
        {
            try
            {
                if (Globals.TCPIP_CCD1.IsOn == false)
                {
                    Globals.TCPIP_CCD1.IP = "127.0.0.1";
                    Globals.TCPIP_CCD1.Port = 8989;
                    Globals.TCPIP_CCD1.IsReconnection = true;
                    Globals.TCPIP_CCD1.ReConnectTime = 6000;
                    Globals.TCPIP_CCD1.Start();
                }
                return true;
            }
            catch { return false; }  
        }
        private bool CCD1_Close()
        {
            try
            {
                if (Globals.TCPIP_CCD1.IsOn)
                {
                    Globals.TCPIP_CCD1.Close();
                }
                return true;
            }
            catch { return false; }
        ...
        #endregion
        #region 条码枪通讯
        private bool Scanner_Connect()
        {
            try
            {
                if (Globals.TCPIP_Scanner.IsOn == false)
                {
                    Globals.TCPIP_Scanner.IP = "169.254.185.116";
                    Globals.TCPIP_Scanner.Port = 23;
                    Globals.TCPIP_Scanner.IsReconnection = true ;
                    Globals.TCPIP_Scanner.ReConnectTime = 6000;
                    Globals.TCPIP_Scanner.Start();
                }
                return true;
            }
            catch { return false; }
        }
        private bool Scanner_Close()
        {
            try
            {
                if (Globals.TCPIP_Scanner.IsOn)
                {
                    Globals.TCPIP_Scanner.Close();
                }
                return true;
            }
            catch { return false; }

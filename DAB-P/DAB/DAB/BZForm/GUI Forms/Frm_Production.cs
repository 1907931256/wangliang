...
namespace DAB
{
    public partial class Frm_Production
    {
        ...
        internal System.Timers.Timer Chart_Time = new System.Timers.Timer() { AutoReset = true, Interval = 20, Enabled = false };
        ...
        private void Chart_Time_Elapsed(object sender, EventArgs e)
        {
            lock (LockDraw)
                try
                {
                this.BeginInvoke(new Action(() => { Frm_Engineering.fEngineering.Update_DataChart(); })); //刷新压力曲线
                }
                catch (Exception)
                {
